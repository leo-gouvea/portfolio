  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 backdrop-blur-md bg-[color:var(--surface-2)]/80 border-b border-[color:var(--border)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 grid grid-cols-[auto_1fr_auto] items-center gap-3">
        <a href="#top" className="flex items-center gap-2 shrink-0" aria-label={profile.shortName}>
          ...
          <span className="hidden sm:block font-display tracking-[0.3em] text-[color:var(--primary)] text-sm truncate">
            {profile.shortName.toUpperCase()}
          </span>
        </a>

        <nav aria-label="Primary" className="hidden lg:flex items-center justify-center gap-1 min-w-0">
          {links.map((l) => (
            <a
              key={l.href}
              href={l.href}
              className="px-3 py-1.5 font-display tracking-[0.2em] text-xs text-[color:var(--text)] opacity-80 hover:opacity-100 hover:text-[color:var(--primary)] transition-colors"
            >
              {t(l.key)}
            </a>
          ))}
